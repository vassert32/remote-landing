...
Репозиторий — источник правды. Ветки:

| Ветка | Куда едет | Когда |
|---|---|---|
| `main` | прод, https://remote-mentor.ru | автоматом при каждом пуше |
| `preview` | превью, https://vassert32.github.io/remote-landing/ | автоматом при каждом пуше |

Порядок работы для правок текста: коммит в `preview` → смотрим на превью →
мёрж в `main` → через минуту это на боевом домене.

**Править файлы на сервере руками нельзя** — следующая выкатка сносит
каталог целиком и разворачивает сборку заново. Всё меняется в репозитории.

Тексты сайта живут в одном месте: `src/i18n/ru.ts` и `src/i18n/en.ts`.
Больше нигде видимых строк нет, править можно прямо в веб-интерфейсе GitHub.
...
Обычный путь — пуш в `main`, дальше `.github/workflows/deploy.yml` всё
делает сам: собирает, заливает, проверяет что прод отвечает и что битый
адрес отдаёт честный 404. Ключ лежит в секрете репозитория `VPS_SSH_KEY`.

Руками (если Actions недоступны):
...
Собирается автоматически при пуше в ветку `preview` — песочница для проверки
